  ...
  return (
    <main className='rapports'>
          ...
          {ventesFilter.map((item) => (
            <tbody key={item._id}>
              <tr className='li_2'>
                ...
                <th className='co2'>{item.prix_vente * item.qty} FCFA</th>
                <th className='co2'>
                  {((item.prix_vente * item.qty / item.qty) - item.prix_achat) * item.qty} FCFA
                </th>
              </tr>
            </tbody>
          ))}
      ...
      <section className='infos-ventes'>
        ...
        <section className='bene'>
          <p>LE BENEFICE TOTAL</p>
          <span>{beneficeTotal} FCFA</span>
        </section>
        <p>
          Rapports{' '}
          {dateValue
            ? `du ${new Date(datevente?.date_vente).toLocaleDateString('fr-FR', {
                month: 'long',
                day: 'numeric',
                year: 'numeric',
              })}`
        ...
        <button className='btn-print' onClick={() => handlePrint()}>
          <PrintIcon style={{ marginRight: 10 }} /> Imprimer
        </button>
      </section>
    </main>
  );
