  ...
  return (
    ...
    <main className='rapports'>
          ...
          {ventesFilter.map((item) => (
            <tbody key={item._id}>
              <tr className='li_2' >
                ...
                <th className='co2'>{((item.prixVente * item.qty / item.qty) - item.prixAchat) * item.qty} FCFA</th>
              </tr>
      ...
      <section className='infos-ventes'>
        ...
        <p>Rapports {dateValue ? `du ${new Date(datevente?.timestamps).toLocaleDateString('fr-FR', { month: 'long', day: 'numeric', year: 'numeric' })}`  : "de tous les ventes"}</p>
